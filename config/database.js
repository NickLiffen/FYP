...
module.exports = {
    ...
    teacherCurrentClassStatus: function(ID){
      return new Promise(function(resolve, reject) {
          connection.query(`SELECT LOWER( Class.Class_ID ) AS 'Class_ID', Subject.Subject_Name AS 'Subject_Name', LOWER(  Class.Class_Start_Timestamp ) AS 'Class Start Time', LOWER(  Class.Class_End_Timestamp ) AS 'Class End Time', LOWER(  Class.Class_Level ) AS 'Class Level', TIMEDIFF(NOW() + INTERVAL 1 HOUR, Class.Class_Start_Timestamp) AS 'Time_Difference', CASE when Attendance.Attendance_Status is null then 'No' else 'Yes' end as 'Has_Attendance_Been_Taken' FROM Class LEFT JOIN Attendance ON(Attendance.Class_id = Class.Class_ID) INNER JOIN Teacher ON(Teacher.Teacher_ID = Class.Teacher_ID) INNER JOIN Subject ON(Subject.Subject_ID = Class.Subject_ID) INNER JOIN Room ON(Room.Room_ID = Class.Room_ID) WHERE Teacher.Teacher_ID = ${ID} AND NOW() + INTERVAL 1 HOUR between class.class_start_timestamp and class.class_end_timestamp LIMIT 1;`, function(err, results) {
              //If error reject the promise.
              if (err) {
                  console.log(err);
                  reject(Error(err));
        ...
        return new Promise(function(resolve, reject) {
          ...
          getParentStudents: function(parentID){
            return new Promise((resolve, reject) => {
              let sqlStatement = `SELECT Student.Student_ID as 'Student_ID', CONCAT( Student.Student_Fname, ' ' , Student.Student_Lname)  AS 'Student_Name', Student.Student_Email AS 'Student_Email', Student.Student_Year AS 'Student_Year', Student.Student_Username AS 'Student_Username', CASE when Subject.Subject_ID is null then 'Student Has No Class' else Subject.Subject_Name end as 'Subject_Name', CASE when Attendance.Attendance_Status is null then 'No Attendance Taken Yet' else Attendance.Attendance_Status end as 'Attendance_Status' FROM Student LEFT JOIN Student_Has_Class ON(student_has_class.student_id = student.student_id) LEFT JOIN class ON class.class_id = student_has_class.class_id LEFT JOIN Subject ON(class.subject_id = subject.subject_id) LEFT JOIN Attendance ON(Attendance.class_id = class.class_ID AND Attendance.student_id = student.student_id) LEFT JOIN Student_has_Parent ON (Student_has_Parent.Student_Student_ID = Student.Student_ID) LEFT JOIN Parent ON (Parent.Parent_ID = Student_has_Parent.Parent_Parent_ID) WHERE Parent.Parent_ID = '${parentID}' AND NOW() + INTERVAL 1 HOUR between class.class_start_timestamp AND class.class_end_timestamp;`;
