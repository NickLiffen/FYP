...
function createSubjectTable() {
    connection.query({
        sql: "CREATE TABLE IF NOT EXISTS `Subject` ( `Subject_ID` INT NOT NULL AUTO_INCREMENT, `Subject_Name` VARCHAR(45) NOT NULL, `Subject_Description` VARCHAR(45) NULL, PRIMARY KEY (`Subject_ID`), UNIQUE INDEX `Subject_ID_UNIQUE` (`Subject_ID` ASC)) ENGINE = InnoDB;",
        timeout: 4000,
    }, function(error) {
        if (error) {
            console.log(error);
...
function createRoomTable() {
    connection.query({
        sql: "CREATE TABLE IF NOT EXISTS `Room` ( `Room_ID` INT NOT NULL AUTO_INCREMENT, `Room_Name` VARCHAR(45) NOT NULL, `Rom_Description` VARCHAR(45) NOT NULL, PRIMARY KEY (`Room_ID`), UNIQUE INDEX `Room_ID_UNIQUE` (`Room_ID` ASC)) ENGINE = InnoDB;",
        timeout: 5000,
    }, function(error) {
        if (error) {
            console.log(error);
...
function createAdminTable() {
    connection.query({
        sql: "CREATE TABLE IF NOT EXISTS `Admin` ( `Admin_ID` INT NOT NULL AUTO_INCREMENT, `Admin_Title` VARCHAR(45) NOT NULL, `Admin_Fname` VARCHAR(45) NOT NULL, `Admin_Lname` VARCHAR(45) NOT NULL, `Admin_Email` VARCHAR(45) NOT NULL, `Admin_Privledge_Level` VARCHAR(45) NULL, `Admin_Username` VARCHAR(45) NOT NULL, `Admin_Password` VARCHAR(300) NULL, `Role` VARCHAR(45) NOT NULL, PRIMARY KEY (`Admin_ID`), UNIQUE INDEX `Admin_ID_UNIQUE` (`Admin_ID` ASC)) ENGINE = InnoDB;",
        timeout: 7000,
    }, function(error) {
        if (error) {
            console.log(error);
...
function createStudentHasClassTable() {
    connection.query({
        sql: "CREATE TABLE IF NOT EXISTS `Student_has_Class` ( `Student_ID` INT NOT NULL, `Class_ID` INT NOT NULL, PRIMARY KEY (`Student_ID`, `Class_ID`), INDEX `fk_Student_has_Class_Class1_idx` (`Class_ID` ASC), INDEX `fk_Student_has_Class_Student_idx` (`Student_ID` ASC), CONSTRAINT `fk_Student_has_Class_Student` FOREIGN KEY (`Student_ID`) REFERENCES `testFYP`.`Student` (`Student_ID`) ON DELETE NO ACTION, CONSTRAINT `fk_Student_has_Class_Class1` FOREIGN KEY (`Class_ID`) REFERENCES `testFYP`.`Class` (`Class_ID`) ON DELETE NO ACTION) ENGINE = InnoDB;",
        timeout: 8000,
    }, function(error) {
        if (error) {
            console.log(error);
